...
- 后端会为新书生成 `book.json`、`chapters`、默认 `idea.md`、`plot.md`、`novel-spec.md`、`story-status.md` 和 `outline/`。
- `plot.md` 用于长期剧情走向、主线支线、伏笔回收、反转和剧情点子池；`novel-spec.md` 只管规格、文风、篇幅和创作规则。
### 创作台
- 展示单本书的项目文件树。
...
- AI 相关变更需要考虑文件刷新、流式响应、编辑器脏状态，以及 `plot.md`、`story-status.md`、`outline/` 的职责边界。
### 本地持久化与模型
- 后端直接读写本地文件系统。
